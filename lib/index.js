...
MongoClient.connect(mongodbConfig.url, (err, mongoPool) => {
	// seed data
	// mongodb(mongoPool).seedSportsData();
	...
	app.use('/graphql', (req, res) => {
		const loaders = {
			...
			mongodb: {
				...
				popularEvents: new DataLoader(mongodb(mongoPool).getPopularEvents),
				popularCompetitions: new DataLoader(mongodb(mongoPool).getPopularCompetitions),
				liveEvents: new DataLoader(mongodb(mongoPool).getLiveEvents),
				sportsbookPageStatic: new DataLoader(mongodb(mongoPool).getSportsbookPageStatic)
			}
		};
		graphqlHTTP({
			schema: ncSchema,
